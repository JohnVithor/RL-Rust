...
impl<Obs, Action> ContinuousObsDiscreteTrainer<Obs, Action> {
    pub fn new(repr_to_action: fn(usize) -> Action, obs_to_repr: fn(&Obs) -> Tensor) -> Self {
        Self {
            repr_to_action,
            obs_to_repr,
        ...
        Obs: Clone,
    {
        ...
        let mut curr_obs: Tensor = (self.obs_to_repr)(&env.reset());
        let mut curr_action = agent.get_action(&curr_obs);

        for _ in 0..n_steps {
            let (next_obs, reward, terminated) =
                env.step((self.repr_to_action)(curr_action)).unwrap();
            ...
            if terminated {
                ...
                if n_episodes % eval_at == 0 {
                    let (rewards, eval_lengths) = self.evaluate(env, agent, eval_for);
                ...
                curr_obs = (self.obs_to_repr)(&env.reset());
