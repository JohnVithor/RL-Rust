...
impl GymEnv {
    ...
    pub fn reset(&self) -> PyResult<Tensor> {
        let gil = Python::acquire_gil();
        let py = gil.python();
        let obs_info = self.env.call_method(py, "reset", NoArgs, None)?;
        let obs = obs_info.get_item(py, 0)?;
        Ok(Tensor::from_slice(&obs.extract::<Vec<f32>>(py)?))
    }

    /// Applies an environment step using the specified action.
    pub fn step<A: ToPyObject + Copy + Debug>(&self, action: A) -> PyResult<Step<A>> {
        let gil = Python::acquire_gil();
        let py = gil.python();
...
pub fn run() -> cpython::PyResult<()> {
    ...
    let num_actions = 1; //env.action_space() as usize;
    ...
    for episode in 0..MAX_EPISODES {
        ...
        for _ in 0..EPISODE_LENGTH {
            let actions = f64::try_from(agent.actions(&obs)).unwrap();

            let action_vec = actions.clamp(0.0, 1.0).round() as u32;
